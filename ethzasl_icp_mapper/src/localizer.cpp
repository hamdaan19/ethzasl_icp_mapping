#include "ethzasl_icp_mapper/localizer.h"

Localizer::Localizer(ros::NodeHandle& n, ros::NodeHandle& pn):
	n(n),
	pn(pn),
	mapPointCloud(0),
	...
	eps(0.0001)
{
	...
	getModeSrv = pn.advertiseService("get_mode", &Localizer::getMode, this);
	getBoundedMapSrv = pn.advertiseService("get_bounded_map", &Localizer::getBoundedMap, this);
	reloadAllYamlSrv= pn.advertiseService("reload_all_yaml", &Localizer::reloadallYaml, this);

	// refreshing tf transform thread
	publishThread = boost::thread(boost::bind(&Localizer::publishLoop, this, tfRefreshPeriod));
...
void Localizer::processCloud(unique_ptr<DP> newPointCloud, const std::string& scannerFrame, const ros::Time& stamp, uint32_t seq)
{
	...
	try 
	{
		...
		)
		{
			...
			mapBuildingTask = MapBuildingTask(boost::bind(&Localizer::updateMap, this, newPointCloud.release(), T_updatedScanner_to_map, true));
...
void Localizer::processNewMapIfAvailable()
{
	#if BOOST_VERSION >= 104100
...
void Localizer::setMap(DP* newMapPointCloud)
{
...
Localizer::DP* Localizer::updateMap(DP* newPointCloud, const PM::TransformationParameters T_updatedScanner_to_map, bool mapExists)
{
	timer t;
...
void Localizer::waitForMapBuildingCompleted()
{
	#if BOOST_VERSION >= 104100
	if (mapBuildingInProgress)
	{
...
void Localizer::publishLoop(double publishPeriod)
{
	if(publishPeriod == 0)
		return;
...
void Localizer::publishTransform()
{
...
bool Localizer::getPointMap(map_msgs::GetPointMap::Request &req, map_msgs::GetPointMap::Response &res)
{
	if (!mapPointCloud)
		return false;
...
bool Localizer::correctPose(ethzasl_icp_mapper::CorrectPose::Request &req, ethzasl_icp_mapper::CorrectPose::Response &res)
{
	publishLock.lock();
...
bool Localizer::setMode(ethzasl_icp_mapper::SetMode::Request &req, ethzasl_icp_mapper::SetMode::Response &res)
{
	// Impossible states
...
bool Localizer::getMode(ethzasl_icp_mapper::GetMode::Request &req, ethzasl_icp_mapper::GetMode::Response &res)
{
...
bool Localizer::getBoundedMap(ethzasl_icp_mapper::GetBoundedMap::Request &req, ethzasl_icp_mapper::GetBoundedMap::Response &res)
{
	if (!mapPointCloud)
		return false;
...
void Localizer::loadExternalParameters()
{
...
bool Localizer::reloadallYaml(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
{
...
// Main function supporting the Localizer class
// int main(int argc, char **argv)
// {
// 	ros::init(argc, argv, "mapper");
// 	ros::NodeHandle n;
// 	ros::NodeHandle pn("~");
// 	Localizer mapper(n, pn);
// 	ros::spin();
